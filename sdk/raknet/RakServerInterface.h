...
public:
	...
	virtual void RegisterAsRemoteProcedureCall( UniqueID uniqueID, void ( *functionPointer ) ( RPCParameters *rpcParms ) )=0;
	...
	virtual void RegisterClassMemberRPC( UniqueID uniqueID, void *functionPointer )=0;
	...
	virtual void UnregisterAsRemoteProcedureCall( UniqueID uniqueID )=0;
	...
	virtual bool RPC( UniqueID uniqueID, const char *data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp, NetworkID networkID, RakNet::BitStream *replyFromTarget )=0;
	...
	/// \param[in] replyFromTarget If 0, this function is non-blocking.  Otherwise it will block while waiting for a reply from the target procedure, which is remtely written to RPCParameters::replyToSender and copied to replyFromTarget.  The block will return early on disconnect or if the sent packet is unreliable and more than 3X the ping has elapsed.
	/// \return True on a successful packet send (this does not indicate the recipient performed the call), false on failure
	virtual bool RPC( UniqueID uniqueID, RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp, NetworkID networkID = UNASSIGNED_NETWORK_ID, RakNet::BitStream *replyFromTarget = NULL )=0;
	...
	virtual void AddToBanList( const char *IP, RakNet::Time ms = 0U ) = 0;
