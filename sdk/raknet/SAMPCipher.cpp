
#include "SAMPCipher.h"
#include <string.h>

unsigned short xor_key = 0;

unsigned char hash_table[] = {
#ifdef SAMPSRV
	
#else
	
#endif
};

void SetXorKey(int key)
{
	xor_key = key ^ 0xCCCC;
}

unsigned char GetDataChecksum(char* data, int len)
{
	unsigned char sum = 0;
	for (int i = 0; i != len; i++)
	{
		sum ^= data[i] & 0xAA;
	}
	return sum;
}

void Transform(unsigned char low, unsigned char high, unsigned char* data, int len)
{
	bool switcher = false;
	for (int i = 0; i != len; i++)
	{
#ifndef SAMPSRV
		data[i] = hash_table[data[i]];
#endif
		if (switcher)
		{
			data[i] ^= high;
			switcher = false;
		}
		else
		{
			data[i] ^= low;
			switcher = true;
		}
#ifdef SAMPSRV
		data[i] = hash_table[data[i]];
#endif
	}
}

#ifdef SAMPSRV

bool DecryptData(char* dest, char* src, int* len)
{
	unsigned char high = (unsigned char)xor_key;
	unsigned char check = src[0];
	*len -= 1;
	memcpy(dest, src + 1, *len);
	Transform(0, high, (unsigned char*)dest, *len);
	return check == GetDataChecksum(dest, *len);
}

#else

void EncryptData(char* dest, char* src, int* len)
{
	unsigned char high = (unsigned char)xor_key;
	dest[0] = GetDataChecksum(src, *len);
	memcpy(dest + 1, src, *len);
	Transform(0, high, (unsigned char*)(dest + 1), *len);
	*len += 1;
}

#endif