...
CMenuPool::CMenuPool()
{
	...
	for (WORD wPlayer = 0; wPlayer < MAX_PLAYERS; wPlayer++)
	{
		m_bytePlayerMenu[wPlayer] = INVALID_MENU_ID;
	}
}
...
{
	if (byteMenuID >= MAX_MENUS || m_bMenuSlotState[byteMenuID] == false || !m_pMenus[byteMenuID])
	{
		return false;
	}
	m_bMenuSlotState[byteMenuID] = false;
