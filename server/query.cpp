...
extern RakServerInterface* pRak; // from netrpc.cpp
...
DWORD dwLastPingQueryTick=0;

int CheckPingQueryFlood(unsigned int binaryAddress)
{
	if (!dwLastPingQueryTick) {
		dwLastPingQueryTick = pNetGame->GetTime();
		return 0;
	}
	if ((pNetGame->GetTime() - dwLastPingQueryTick) < 1) {
		return 1;
	}
	dwLastPingQueryTick = pNetGame->GetTime();
	return 0;
...
DWORD dwLastRulessQueryTick=0;

int CheckRulesQueryFlood(unsigned int binaryAddress)
{
	if (!dwLastRulessQueryTick) {
		dwLastRulessQueryTick = pNetGame->GetTime();
		return 0;
	}
	if ((pNetGame->GetTime() - dwLastRulessQueryTick) < 25) {
		return 1;
	}
	dwLastRulessQueryTick = pNetGame->GetTime();
	return 0;
...
DWORD dwLastInfoQueryTick=0;

int CheckInfoQueryFlood(unsigned int binaryAddress)
{
	if (!dwLastInfoQueryTick) {
		dwLastInfoQueryTick = pNetGame->GetTime();
		return 0;
	}
	if ((pNetGame->GetTime() - dwLastInfoQueryTick) < 2) {
		return 1;
	}
	dwLastInfoQueryTick = pNetGame->GetTime();
	return 0;
...
{
	if (!dwLastQueryTick) {
		dwLastQueryTick = pNetGame->GetTime();
	...
	//if (lastBinAddr == binaryAddress) {
		//return 0;
	//}
	if ((pNetGame->GetTime() - dwLastQueryTick) < 25) {
		return 1;
	}
	dwLastQueryTick = pNetGame->GetTime();
	//lastBinAddr = binaryAddress;
...
int ProcessQueryPacket(unsigned int binaryAddress, unsigned short port, char* data, int length, SOCKET s)
{
	...
	if ((length > 4) && (*(unsigned long*)(data) == 0x504D4153)) // 0x504D4153 = "SAMP" as hex.
	{
		...
		if (length >= 11)
		{
			...
			case 'p':	// ping
			{
				if (!CheckPingQueryFlood(binaryAddress) || length == 15)
				{
			...
			case 'i':	// info
			{
				...
				if(CheckInfoQueryFlood(binaryAddress)) return 1;
				...
				WORD wPlayerCount = pPlayerPool->GetPlayerCount();

				WORD wMaxPlayers = pConsole->GetIntVariable("maxplayers");
				wMaxPlayers -= pPlayerPool->GetNPCCount();
			...
			case 'c':	// players
			{
				...
				for (int r=0; r<=pPlayerPool->GetPoolSize(); r++)
				{
					if (pPlayerPool->GetSlotState(r) &&
						!pPlayerPool->IsPlayerNPC(r) &&
						pPlayerPool->IsValidID(r))
					{
			...
			case 'r':	// rules
			{
				// We do not process these queries 'query' is 0
				if (!pConsole->GetBoolVariable("query")) return 1;
				if(CheckRulesQueryFlood(binaryAddress)) return 1;
			...
			case 'x':	// rcon
			{
				...
				// We do not process these queries 'query' is 0
				if(!pConsole->GetBoolVariable("query")) return 1;
				if(!RCONPasswordValid()) return 1;
				if(!pConsole->GetBoolVariable("rcon")) return 1;
				if(CheckQueryFlood(binaryAddress)) return 1;
